                            ...
                            {testCases.map(tc => (
                                <tr key={tc.id} className="hover:bg-muted transition-colors">
                                    ...
                                    <td className="px-4 py-3">
                                        <span className={`font-semibold text-xs ${priorityColor(tc.priority)}`}>{tc.priority || '—'}</span>
                                    </td>
                                    <td className="px-4 py-3">
                                        <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${statusColor(tc.status)}`}>{tc.status || '—'}</span>
                                    </td>
                                    <td className="px-4 py-3 text-muted-foreground text-xs">
                                        {tc.assigned_to
                                            ? (profiles[tc.assigned_to] || tc.assigned_to)
                                            : 'Unassigned'}
                                    </td>
                                </tr>
                            ))}
...
const CommentsTab = ({ storyId, storyUUID }) => {
    ...
    const handlePostComment = async () => {
        ...
        try {
            ...
            const { data, error } = await supabase
                .from('comments')
                .insert([{ story_id: storyUUID, comment_text: newComment.trim(), user_id: userId }])
                .select('*, profiles(full_name, email)');
    ...
    if (!storyUUID) return (
        <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-sm font-medium text-muted-foreground">Save the story first to enable comments</p>
        </div>
    );
            ...
            {comments.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                    <div className="w-14 h-14 bg-muted rounded-full flex items-center justify-center">
                        <i className="fa-solid fa-comment-dots text-2xl text-muted-foreground"></i>
                    </div>
                    <p className="text-sm font-medium text-muted-foreground">No comments yet — be the first!</p>
                </div>
            ) : (
                <div className="space-y-4">
                    {comments.map(c => {
                        ...
                        return (
                            <div key={c.id} className="flex gap-3 group">
                                <div className={`w-9 h-9 ${avatarColor(displayName)} rounded-full flex items-center justify-center flex-shrink-0`}>
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <button onClick={() => handleDelete(c.id)}
                                            className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-red-500 text-xs"
        ...
        <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            ...
            <div style={{ display: 'flex', gap: 4, opacity: hov ? 1 : 0, transition: 'opacity 0.15s' }}>
                <a href={att.file_url} target="_blank" rel="noopener noreferrer"
                    ...
                    title="Download"><i className="fa-solid fa-download" style={{ fontSize: 11 }}></i></a>
                <button onClick={() => onDelete(att)}
                    ...
                    title="Delete"><i className="fa-solid fa-trash" style={{ fontSize: 11 }}></i></button>
            </div>
        </div>
    );
};
...
const AttachmentsTab = ({ storyId, storyUUID }) => {
        ...
        try {
            for (const file of files) {
                ...
                const { error: dbError } = await supabase.from('attachments').insert([{
                    ...
                    file_size: file.size,
                    file_type: file.type,
                    uploaded_by: uploadedBy.trim() || 'Anonymous',
                }]);
    ...
    if (!storyUUID) return (
        <div className="flex flex-col items-center justify-center py-16 gap-3">
            <div className="w-14 h-14 bg-yellow-50 rounded-full flex items-center justify-center"><i className="fa-solid fa-triangle-exclamation text-2xl text-yellow-400"></i></div>
            <p className="text-sm font-medium text-muted-foreground">Save the story first to enable attachments</p>
        </div>
    );
    ...
    const MicroLabel = ({ children, color = '#6B7280' }) => (
        <span style={{ display: 'block', fontFamily: font, ...T.micro, textTransform: 'uppercase', color, marginBottom: 6 }}>{children}</span>
    );
            ...
            <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
                ...
                <div style={{ display: 'flex', gap: 4, opacity: hov ? 1 : 0, transition: 'opacity 0.15s' }}>
                    <a href={href} target="_blank" rel="noopener noreferrer"
                        ...
                        title="Open link"><i className="fa-solid fa-arrow-up-right-from-square" style={{ fontSize: 11 }}></i></a>
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24, fontFamily: font }}>
            ...
            <div style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 14, padding: '20px 20px 16px' }}>
                ...
                {linksLoading
                    ? <div style={{ display: 'flex', justifyContent: 'center', padding: '24px 0' }}><div style={{ width: 20, height: 20, border: '2px solid #7C3AED', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }}></div></div>
                    : figmaLinks.length === 0 && !showFigmaForm
                        ? <EmptyState icon="fa-brands fa-figma" message="No Figma design links added yet" />
                        : <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {figmaLinks.map(link => (
                                <LinkRow key={link.id} href={link.figma_link} url={link.figma_link} icon="fa-brands fa-figma" iconBg="rgba(139,92,246,0.10)" iconColor="#7C3AED" urlColor="#6D28D9" hoverBorder="#C4B5FD" title={link.title || ''} meta={`Added by ${link.added_by || 'Anonymous'} · ${new Date(link.created_at).toLocaleDateString()}`} metaColor="#9CA3AF" onDelete={() => handleDeleteLink(link)} />
                            ))}
                        </div>
                }
            </div>
            ...
            <div style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 14, padding: '20px 20px 16px' }}>
                ...
                {linksLoading
                    ? <div style={{ display: 'flex', justifyContent: 'center', padding: '24px 0' }}><div style={{ width: 20, height: 20, border: '2px solid #2563EB', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }}></div></div>
                    : webLinks.length === 0 && !showWebForm
                        ? <EmptyState icon="fa-solid fa-globe" message="No web links added yet" />
                        : <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {webLinks.map(link => (
                                <LinkRow key={link.id} href={link.web_link} url={link.web_link} icon="fa-solid fa-globe" iconBg="rgba(59,130,246,0.10)" iconColor="#2563EB" urlColor="#1D4ED8" hoverBorder="#93C5FD" title={link.title || ''} meta={`Added by ${link.added_by || 'Anonymous'} · ${new Date(link.created_at).toLocaleDateString()}`} metaColor="#9CA3AF" onDelete={() => handleDeleteLink(link)} />
                            ))}
                        </div>
                }
            </div>
            ...
            <div style={{ background: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: 14, padding: '20px 20px 16px' }}>
                ...
                <div
                    ...
                    style={{ border: '2px dashed #D1FAE5', borderRadius: 12, padding: '28px 20px', textAlign: 'center', marginBottom: 16 }}>
                    ...
                    <p style={{ fontFamily: font, ...T.label, color: '#6B7280', marginBottom: 14 }}>Supports any file type · spaces in filenames are handled automatically</p>
                ...
                {loading
                    ...
                    : attachments.length === 0
                        ...
                        : <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            ...
                            {attachments.map(att => (
                                <AttachmentRow key={att.id} att={att} fileIcon={fileIcon} formatSize={formatSize} onDelete={handleDeleteFile} />
                            ))}
                        </div>
                }
            </div>
...
const UserStoryMapping = () => {
    ...
    useEffect(() => {
        ...
        const fetchCounts = async () => {
            const [bg, cm, at] = await Promise.all([
    ...
    const buildPayload = (extraStatus) => {
        ...
        if (formData.approvedAt) {
            approvedAt = formData.approvedAt.includes('T') ? formData.approvedAt : `${formData.approvedAt}T00:00:00.000Z`;
        }
        return {
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                                <div>
                                                    ...
                                                    <div className="relative">
                                                        ...
                                                        <button onClick={handleStoryIdEditStart} className="absolute right-2.5 top-1/2 -translate-y-1/2 w-6 h-6 flex items-center justify-center rounded text-indigo-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors" title="Edit Story ID">
