...
export default function Sidebar() {
    ...
    // ─── Ref guard: prevents React Strict Mode double-mount from
    // creating two competing Supabase lock subscribers, which causes:
    //   "Lock was not released within 5000ms" + AbortError
    const { role } = useRole();
    const listenerRef = useRef(null);

    useEffect(() => {
        // Block the second mount in React Strict Mode dev double-invoke
        ...
        async function loadProfile() {
            try {
                ...
                // Silently ignore AbortError from unmount during fetch
        ...
        // Subscribe once, store ref so cleanup can unsubscribe
    ...
    // All users can see all pages — role-based filtering disabled
